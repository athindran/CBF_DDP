...
from simulators import(load_config, CarDouble5DEnv, BicycleReachAvoid5DMarginTwo, PrintLogger, Bicycle5DCost)
...
def main(config_file, plot_tag, road_boundary, is_task_ilqr):
  ...
  if config_cost.COST_TYPE == "Reachavoid":
    if config_solver.FILTER_TYPE == "none":
      ...
      policy_type = "iLQRSafetyFilterGame"
      cost = BicycleReachAvoid5DMarginTwo(config_ilqr_cost, copy.deepcopy(env.agent.dyn))
      task_cost = Bicycle5DCost(config_ilqr_cost, copy.deepcopy(env.agent.dyn))
      env.cost = cost  #! hacky
  #Not supported    
  ...
  def rollout_step_callback(
      env: CarDouble5DEnv, state_history, action_history, plan_history, step_history,
      *args, **kwargs
  ):
  ...
  for _, yaw_constraint in enumerate(yaw_constraints):
    for filter_type in ['CBF', 'LR']:
        ...
        cost = BicycleReachAvoid5DMarginTwo(config_current_cost, copy.deepcopy(env.agent.dyn))
        env.cost = cost
        env.agent.init_policy(
          policy_type=policy_type, config=config_solver, cost=cost, task_cost=task_cost
        )
