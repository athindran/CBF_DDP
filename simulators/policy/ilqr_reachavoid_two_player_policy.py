...
class iLQRReachAvoidGame(iLQR):
  ...
  def backward_pass(
    ...
    def failure_backward_func(args):
      ...
      ks = jnp.linalg.solve(Q_uu, Q_u)
      ks1, ks2 = jnp.split(ks, 2, axis=0)

      beta = - fu[:, :, idx]@ks1 - fd[:, :, idx]@ks2
      
      n1 = 0.5*(ks1.T@c_uu[:, :, idx] - 2*c_u[:, idx])@ks1 - (2*V_x - V_xx@(-beta)).T@(-beta) + n1
      n2 = 0.5*(ks2.T@c_dd[:, :, idx] - 2*c_d[:, idx])@ks2 + (2*V_x - V_xx@(-beta)).T@(-beta) + n2
    ...
    def propagate_backward_func(args):
      ...
      V_xx = f_cl.T @V_xx @f_cl + c_xx[:, :, idx] + Ks1.T @ c_uu[:, :, idx] @ Ks1 + Ks2.T @ c_dd[:, :, idx] @ Ks2 
      ...
      V_x = f_cl.T @ (V_x + V_xx@beta) + c_x[:, idx] + Ks1.T@c_uu[:, :, idx]@ks1 - Ks1.T@c_u[:, idx] + Ks2.T@c_dd[:, :, idx]@ks2 - Ks2.T@c_d[:, idx]
    ...
    Ks2 = jnp.zeros((self.dim_u, self.dim_x, self.N - 1))
    ks2 = jnp.zeros((self.dim_u, self.N - 1))
    ...
    V_x, V_xx, ks1, Ks1, ks2, Ks2 = jax.lax.fori_loop(
