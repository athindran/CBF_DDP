...
from simulators.policy.twoplayer.ilqr_reachavoid_two_player_policy import iLQRReachAvoidGame
from simulators.policy.twoplayer.ilqr_two_player_policy import iLQR
from simulators.policy.twoplayer.solver_utils import barrier_filter_linear, barrier_filter_quadratic, bicycle_linear_task_policy
...
class iLQRSafetyFilter(iLQR):
  ...
  def __init__( self, id: str, config, dyn, cost, task_cost ) -> None:
    ...
    if self.config.COST_TYPE=="Reachavoid":
      self.solver_0 = iLQRReachAvoidGame(self.id, self.config, self.rollout_dyn_0, self.cost)
      self.solver_1 = iLQRReachAvoidGame(self.id, self.config, self.rollout_dyn_1, self.cost)
      self.solver_2 = iLQRReachAvoidGame(self.id, self.config, self.rollout_dyn_1, self.cost)
  ...
  def get_action(
    ...
    if prev_sol is None or prev_sol['resolve']:
      ...
      control_0 = solver_info_0['controls'][:, 0] + solver_info_0['Ks1'][:, :, 0] @ (initial_state - solver_info_0['states'][:, 0])
    ...
    state_imaginary, task_ctrl, _ = self.dyn.integrate_forward(
            state=initial_state, control=task_ctrl, disturbance=jnp.zeros((2,))
    )
    ...
    if(self.filter_type=="LR"):
      ...
      while((constraint_violation<cbf_tol or warmup) and num_iters<5):
        ...
        _, B0, _ = self.dyn.get_jacobian( initial_state_jnp , initial_control_jnp, jnp.zeros((2, 1)))
        ...
        state_imaginary, initial_control, _ = self.dyn.integrate_forward(
            state=initial_state, control=initial_control, disturbance=jnp.zeros((2,))
        )
    ...
    if solver_info_0['is_inside_target']:
        ...
        safety_control = solver_info_0['controls'][:, 0] + solver_info_0['Ks1'][:, :, 0] @ (
