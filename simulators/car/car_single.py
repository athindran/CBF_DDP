...
class CarSingle5DEnv(BaseSingleEnv):
    ...
    def __init__(self, config_env, config_agent, config_constraint) -> None:
        ...
        self.reset_rej_sampling = getattr(
            config_env, "RESET_REJ_SAMPLING", True)
        ...
        for key, value in constraints.items():
            if constraint_values is None:
                ...
                constraint_values = np.concatenate(
                    (constraint_values, value), axis=0)
    ...
    def get_samples(self, nx: int, ny: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        """
        xs = np.linspace(
            self.visual_bounds[0, 0], self.visual_bounds[0, 1], nx)
        ys = np.linspace(
            self.visual_bounds[1, 0], self.visual_bounds[1, 1], ny)
        return xs, ys
