...
from learning.kernel_learning import SafetyFilteringModel
    ...
    if config_solver.FILTER_TYPE == "none":
      ...
      print(
        "[{}]: solver returns status {}, margin {:.1e}, future margin {:.1e}, and uses {:.3f}.".format(
            states.shape[1] - 1, solver_info['status'], solver_info['marginopt'], 
            solver_info['marginopt_next'], solver_info['process_time']
        )
      )
  ...
  for _, yaw_constraint in enumerate(yaw_constraints):
    for filter_type in ['CBF']:
        ...
        if run_first_policy:
          ...
          # GP training
          training_data ={}
          training_data['input_states'] = []
          training_data['targets_V'] = []
          training_data['targets_Vgrad'] = []

          for solver_info in traj_info['plan_history']:
            training_data['input_states'].append( solver_info['input_0'] )
            training_data['targets_V'].append( solver_info['target_01'] )
            training_data['targets_Vgrad'].append( solver_info['target_00'] )
            training_data['input_states'].append( solver_info['input_1'] )
            training_data['targets_V'].append( solver_info['target_11'] )
            training_data['targets_Vgrad'].append( solver_info['target_10'] )
        ...
        n_learning_iters = 0
        while n_learning_iters<1:
          n_learning_iters = n_learning_iters + 1
          model_out_folder = os.path.join(current_out_folder, 'model_fit')
          config_solver.OUT_FOLDER = model_out_folder
          fig_folder = os.path.join(model_out_folder, "figure")
          fig_prog_folder = os.path.join(fig_folder, "progress")
          os.makedirs(fig_prog_folder, exist_ok=True)
          copyfile(config_file, os.path.join(model_out_folder, 'config.yaml'))
          sys.stdout = PrintLogger(os.path.join(config_solver.OUT_FOLDER, 'log.txt'))
          sys.stderr = PrintLogger(os.path.join(config_solver.OUT_FOLDER, 'log.txt'))
          cost = BicycleReachAvoid5DMargin(config_current_cost, copy.deepcopy(env.agent.dyn))

          env.cost = cost
          config_solver.BARRIER_GAMMA = 0.9
          env.agent.init_policy(
            policy_type='ILQRKernelSafetyFilter', config=config_solver, 
              cost=cost, task_cost=task_cost, filtering_model=filtering_model
          )
