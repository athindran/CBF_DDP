...
def main(config_file, plot_tag, road_boundary, is_task_ilqr):
  ...
  x_cur = np.array(getattr(config_solver, "INIT_STATE", [0., 0., 0.5, 0.]))
  env.reset(x_cur)
  ...
  if config_cost.COST_TYPE == "Reachavoid":
    if config_solver.FILTER_TYPE == "none":
      ...
      cost = IntegratorReachability2DMargin(config_ilqr_cost, copy.deepcopy(env.agent.dyn))
      task_cost = Integrator2DCost(config_ilqr_cost, copy.deepcopy(env.agent.dyn))
      env.cost = cost
    else:
  ...
  def rollout_step_callback(
      env: CarIntegrator4DEnv, state_history, action_history, plan_history, step_history,
      *args, **kwargs
  ):
  ...
  for _, yaw_constraint in enumerate(yaw_constraints):
    for filter_type in ['CBF', 'LR']:
        ...
        cost = IntegratorReachability2DMargin(config_current_cost, copy.deepcopy(env.agent.dyn))
        env.cost = cost
        env.agent.init_policy(
          policy_type=policy_type, config=config_solver, cost=cost, task_cost=task_cost
        )
